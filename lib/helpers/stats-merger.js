...
export function mergeCampaignStatsWithMeta(results, campaigns, smsCampaigns, segments = [], lists = [], tags = []) {
    ...
    for (const seg of segments) {
        audienceMap[seg.id] = {
            id: seg.id,
            type: 'segment',
            name: seg.attributes?.name || 'Unknown Segment'
        };
    }
    ...
    return results
        ...
        .map(r => {
            ...
            const campaignMessageId = campaign?.relationships?.['campaign-messages']?.data?.[0]?.id || null;
            ...
            return {
                ...
                groupings: {
                    ...r.groupings,
                    campaign_message_id: campaignMessageId || r.groupings.campaign_message_id || null
                },
                statistics, // Use our normalized statistics object
                campaign_name: campaign?.attributes?.name || null,
                included_audiences: includedAudienceIds.map(id => audienceMap[id]).filter(Boolean),
                excluded_audiences: excludedAudienceIds.map(id => audienceMap[id]).filter(Boolean),
                tagIds,
