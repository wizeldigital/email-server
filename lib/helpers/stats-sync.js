...
export async function klaviyoUpdateStats(store, date = null) {
    ...
    try {
        const [campaigns, smsCampaigns, tags, flows, segments, lists, campaignValuesReports, flowSeriesReports] = await Promise.all([
            klaviyoGetAll(`campaigns?filter=equals(messages.channel,'email'),greater-or-equal(created_at,${fromDate}),equals(status,'Sent')&include=tags`, { apiKey: store.klaviyo_integration.apiKey }),
